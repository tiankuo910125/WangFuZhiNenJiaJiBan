...
public class ProgressView extends View {
    ...
    private void obtionAttrs(Context context, AttributeSet attrs) {
        ...
        mCircleHeight = a.getDimensionPixelSize(R.styleable.CircleProgress_circle_height, 0);
        mCircleWidth = a.getDimensionPixelSize(R.styleable.CircleProgress_circle_width, 0);
    ...
    private void init() {
        ...
        mTextPainTitle.setAntiAlias(true);
        mTextPainTitle.setColor(Color.BLACK);
        //mTextPainTitle.setShadowLayer(5, 2, 2, Color.GRAY);
        mTextPainTitle.setStrokeCap(Paint.Cap.ROUND);
        mTextPainTitle.setTextSize(mFontSize);
        //mTextPainTitle.setTypeface(Typeface.DEFAULT_BOLD);
        textWidth = mTextPainTitle.measureText(mProgress + "%");

        Paint.FontMetrics fontMetrics = mTextPainTitle.getFontMetrics();
    ...
    private void setProgress(float mProgress) {
        ...
        textWidth = mTextPainTitle.measureText((int)mProgress + "%");
        invalidate();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        oval = new RectF(40, 40, mCircleWidth - 40, mCircleHeight - 40);
        ...
        if(withText)
            canvas.drawText((int) mProgress + "%", ( getWidth() - textWidth) / 2, (getWidth() - fontTotalHeight) / 2, mTextPainTitle);
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if ((w > 0) && (h > 0) && ((w != oldw) || (h != oldh))) {
            ...
            oval.set(left + CIRCLE_PADDING, top + CIRCLE_PADDING, w - right - CIRCLE_PADDING, w - bottom - CIRCLE_PADDING);
        }
    }
